...
# 화면에 함께 렌더링되는 라벨과 문구
labels: [STORAGE, CONSISTENCY, 관찰, TRADE-OFF, RESULT]
storage_tradeoff: GPU 정보의 즉시성은 낮아졌지만, 모니터링 서버의 저장 여력을 확보했습니다.
consistency_observation: GPU가 제거·교체된 뒤에도 이전 정보가 남아 실제 구성과 화면이 달라졌습니다.
consistency_action: gpu 테이블에 활성 상태 컬럼을 추가하고, Ansible 수집 시 제거된 GPU는 false로 갱신했습니다.
consistency_result: 현재 구성에 존재하는 GPU만 표시해 실제 서버 구성과 대시보드의 정합성을 유지했습니다.
